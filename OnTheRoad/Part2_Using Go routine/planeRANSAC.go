...
func TripletOfPointsGenerator(in chan Point3D, stop chan bool) chan []Point3D {
    ...
    go func(){
        ...
        for{
            ...
            case out <- []Point3D{pointa,pointb,pointc}:
...
func TakeN(in chan []Point3D, stop chan bool) chan []Point3D {

    out := make(chan []Point3D,numOfIteration)
    ...
    go func() {
        ...
        //one for point gen one for Triplet gen
        stop <- true
        stop <- true
...
func PlaneEstimator(in chan []Point3D) chan Plane3D {
    ...
    out := make(chan Plane3D,numOfIteration)
    go func() {
...
func SupportingPointFinder(in chan Plane3D, pointcloud []Point3D) chan Plane3DwSupport {
...
func FanIn(in [] chan Plane3DwSupport) chan Plane3DwSupport{
    ...
    //wait for all go routine done and close the channel
    go func() {
        wg.Wait()
        close(out)
    }()
...
func DominantPlaneIdentifier(in chan Plane3DwSupport, bestSupport *Plane3DwSupport) {
    ...
    for support := range in {
        ...
        if support.SupportSize > bestSupport.SupportSize {
            ...
            bestSupport.Plane3D = support.Plane3D

            bestSupport.SupportSize = support.SupportSize
...
//for testing each part

        pointChannel := RandomPointGenerator(stop)
        // fmt.Println(<- pointChannel)
        pointList := TripletOfPointsGenerator(pointChannel, stop)
        // fmt.Println(<- pointList)

        retakePointList := TakeN(pointList, stop)
        // fmt.Println(<- retakePointList)

        plane := PlaneEstimator(retakePointList)
        // fmt.Println(<- plane)
        // fmt.Println(<- plane)
        // fmt.Println(<- plane)

        support := make([]chan Plane3DwSupport, numOfIteration)
        for i := 0; i < numOfIteration; i++ {
            support[i] = SupportingPointFinder(plane,pointcloud)
        }
        // fmt.Println(<- support[0])

        fanIn := FanIn(support)
        // fmt.Println(<-fanIn)
        // fmt.Println(<-fanIn)
        DominantPlaneIdentifier(fanIn, &bestSupport)
    ...
    timeused := finishedtime.Sub(currentTime) 

    fmt.Println("Used number of threads: ", numberOfThreads)
    fmt.Println("Time for running the program: ", timeused)
